 ...
 * @brief Advent of Code (AOC) 2015 Puzzle solution for:  Day 06 Part 2.
 *
...
int totalLightsOn(std::vector<std::vector<int>> const &light_status)
{
    int result{0};
...
/// @brief Alter the current light setting to increase by 2
/// @param light_status reference to a int grid (matrix) of lights to be controlled
/// @ return no return value (void)
void toggleLights(std::vector<std::vector<int>> &light_status)
{
    ...
    for (size_t i{0}; i < light_status.size(); i++) {
        for (size_t j{0}; j < light_status[i].size(); j++) {
            ...
            light_status[i][j] = light_status[i][j] + 2;
        }
    }
}

/// @brief Alter the current light setting to it opposite setting
/// @param light_status reference to a boolean grid (matrix) of lights to be controlled
/// #param points of int point_i{0}, point_j{0}, point_k{0}, point_m{0};
/// @ return no return value (void)
void LightsAsActionToggle(std::vector<std::vector<int>> &light_status, std::vector<int> points)
{
    if (debug) {
    ...
    for (size_t i = points.at(0); i <= points.at(2); i++) {
        for (size_t j = points.at(1); j <= points.at(3); j++) {
            // light_status.at(i).at(j) = ight_status.at(i).at(j) + 2; // bounds check version
            light_status[i][j] = light_status[i][j] + 2;
        }
    }
}

/// @brief Alter the current light setting to increase by 1
/// @param light_status reference to a int grid (matrix) of lights to be controlled
/// #param points of int point_i{0}, point_j{0}, point_k{0}, point_m{0};
/// @ return no return value (void)
void LightsAsActionOn(std::vector<std::vector<int>> &light_status, std::vector<int> points)
{
    if (debug) {
...
/// @brief Alter the current light setting to decrease by 1 to min of 0
/// @param light_status reference to a int grid (matrix) of lights to be controlled
/// #param points of int point_i{0}, point_j{0}, point_k{0}, point_m{0};
/// @ return no return value (void)
void LightsAsActionOff(std::vector<std::vector<int>> &light_status, std::vector<int> points)
{
    if (debug) {
...
int main()
{
    ...
    // 0-999 grid if int to track light brightness status - initialised to off
    std::vector<std::vector<int>> light_status{1000, std::vector<int>(1000, 0)};
    ...
    std::cout << "Initial total lights brightness: " << lights_on << '\n';
    ...
    while (std::getline(input_file, line)) {
        ...
        if (debug) {
            std::cout << "Total light brightness: " << totalLightsOn(light_status) << '\n';
        }
    }
    ...
    std::cout << std::endl << "Advent Of Code 2015 :  Day 06 Part 02" << '\n' << '\n';
    std::cout << "  » Number of switches analysed : '" << count_changes << "'" << '\n';
    std::cout << "  » PUZZLE ANSWER: total brightness of all lights combined is: " << lights_on << '\n' << std::endl;

    // Part 2 Answer : 14110788
