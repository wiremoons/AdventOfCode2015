...
int main()
{
    ...
    while (std::getline(input_file, line)) {
        for (auto &ch : line) {
            if (ch == '>') santa ? coords_v.push_back({santa_x++,santa_y}) : coords_v.push_back({robot_x++,robot_y});
            if (ch == '<') santa ? coords_v.push_back({santa_x--,santa_y}) : coords_v.push_back({robot_x--,robot_y});
            if (ch == '^') santa ? coords_v.push_back({santa_x,santa_y++}) : coords_v.push_back({robot_x,robot_y++});
            if (ch == 'v') santa ? coords_v.push_back({santa_x,santa_y--}) : coords_v.push_back({robot_x,robot_y--});
            //std::cout << "Delivery by: " << (santa ? "santa" : "robot");
            //std::cout << " at co-ords: " << (santa ? santa_x : robot_x) << "," << (santa ? santa_y : robot_y) << std::endl;
            deliveries++;
